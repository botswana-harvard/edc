...
class TestsEntryMetaData(TestCase):
    ...
    def test_creates_meta_data2(self):
        """Meta data is created when visit tracking form is added, each instance set to NEW."""
        self.test_visit = self.test_visit_factory(appointment=self.appointment)
        self.assertEqual(ScheduledEntryMetaData.objects.filter(entry_status=UNKEYED, registered_subject=self.registered_subject).count(), 3)
    ...
    def test_creates_meta_data3(self):
        ...
        self.assertEqual(ScheduledEntryMetaData.objects.filter(entry_status=UNKEYED, registered_subject=self.registered_subject).count(), 2)
    ...
    def test_updates_meta_data6(self):
        ...
        obj = TestScheduledModel1Factory(test_visit=self.test_visit)
        obj.delete()
        self.assertEqual(ScheduledEntryMetaData.objects.filter(entry_status=UNKEYED, registered_subject=self.registered_subject, entry__content_type_map__model='testscheduledmodel1').count(), 1)

    def test_updates_meta_data7(self):
        """Meta data instance linked to the model is created if missing, knows model is KEYED."""
        self.test_visit = self.test_visit_factory(appointment=self.appointment)
        ScheduledEntryMetaData.objects.filter(entry_status=UNKEYED, registered_subject=self.registered_subject, entry__model_name='testscheduledmodel1').delete()
        TestScheduledModel1Factory(test_visit=self.test_visit)
