...
def return_items(request, **kwargs):
    ...
    current = None
    defaults = {}
    container_model = get_model(
        dispatch_container_register.container_app_label,
        dispatch_container_register.container_model_name)
    if not container_model:
        raise TypeError(
            'Dispatch Container model \'{app_label}.{model_name}\' does not exist. Got from '
            'DispatchContainerRegister of id \'{pk}\'.'.format(
                app_label=dispatch_container_register.container_app_label,
                model_name=dispatch_container_register.container_model_name,
                pk=dispatch_container_register.id))
    for item in items:
        current = DispatchContainerRegister.objects.get(id=item)
        if current.producer != previous.producer:
            raise TypeError(
                'All items to be returned must be in the same producer. '
                'Got \'{0}\' and \'{1}\'.'.format(current, previous))
