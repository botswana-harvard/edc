    ...
    def is_ready(self,):
        ...
        if not self.get_container_register_instance().is_ready():
            raise AlreadyReturnedController(
                'This controller has already returned it\'s items. To dispatch new items, create a new instance.')
        return True

    def verify_user_container(self, user_container):
        if not isinstance(user_container, SyncModelMixin):
            raise DispatchContainerError('User container must be an instance of SyncModelMixin')
        ...
        if not cls == self.get_user_container_cls():
            raise DispatchContainerError(
                'User container is not of the correct class for this DispatchController. '
                'Expected {0}.'.format(self.get_user_container_cls()))
        ...
        if self.is_ready():
            if self.verify_user_container(user_container):
                if user_container.is_dispatched_as_item():
                    # if controller is in retry, do not raise an exception and
                    # assume container and item are already dispatched
                ...
                if not self.register_with_dispatch_item_register(user_container):
                    raise DispatchError(
                        'Unable to create a dispatch item register for user container '
                        '{0} {1} to {2}.'.format(
                            user_container._meta.object_name, user_container.object, self.using_destination))
                sys.stdout.write(
                    'dispatched {0} {1} to {2}.'.format(
                        user_container._meta.object_name, user_container, self.using_destination))

    def dispatch_user_items_as_json(
            self, user_items, user_container=None, fk_to_skip=None, additional_base_model_class=None):
        if not user_items:
        ...
        if not user_container.is_dispatched_as_item():
            raise DispatchControllerNotReady(
                'User container {0} has not yet been dispatched as an item to {1}. '
                'Dispatch the user container as an item (to json) before dispatching '
                'other items (to json)'.format(user_container, self.using_destination))
        if self.is_ready():
            if self.verify_user_container(user_container):
                ...
                if isinstance(user_container, DispatchContainerRegister):
                    raise DispatchContainerError(
                        'User container may not be DispatchContainerRegister. Got {0}'.format(user_container))
                # confirm no user_items are already dispatched (but send
                # with user_container to skip the "dispatched within container" check)
                ...
                allowed_models = self._get_allowed_base_models(
                    additional_base_model_class=additional_base_model_class)
                if not issubclass(cls_list[0], allowed_models):
                    raise DispatchItemError(
                        'Model {0} is not a subclass of {1}'.format(cls_list[0], allowed_models))
                else:
                    ...
                    if not_dispatchable:
                        raise DispatchItemError(
                            'All instances must be configured for dispatch. Found {0} '
                            'that are not. Got {1}. See method \'is_dispatchable_model\''.format(
                                len(not_dispatchable), not_dispatchable))
                    already_dispatched_items = []
                    for user_instance in user_items:
                        if user_instance.is_dispatched_as_item(
                                using=self.get_using_source(), user_container=user_container):
                            already_dispatched_items.append(user_instance)
                    if already_dispatched_items:
                        raise AlreadyDispatchedItem(
                            '{0} models are already dispatched. Got {1}'.format(
                                len(already_dispatched_items), already_dispatched_items))
                    # dispatch
                    ...
                    for user_item in user_items:
                        if not self.register_with_dispatch_item_register(user_item, user_container):
                            raise DispatchError(
                                'Unable to create a dispatch item register instance for '
                                '{0} {1} to {2}.'.format(
                                    user_item._meta.object_name, user_item, self.using_destination))
                        sys.stdout.write(
                            '  dispatched user item {0} {1} to {2}.'.format(
                                user_item._meta.object_name, user_item, self.using_destination))

    def _dispatch_as_json(
            self, model_instances, user_container=None, fk_to_skip=None, additional_base_model_class=None):
        """Passes on to _to_json along with a callback to consume foreignkeys.

           Bypass container_dispatch_model if self.skip_container is set
           True, dispatch_container_model is set True"""
        if (self.skip_container and user_container.is_dispatch_container_model() and
                model_instances == user_container):
            sys.stdout.write(
