...
class StudySpecificForm(forms.ModelForm):
        ...
        if not device_id == 0:
            ...
            if not str(real_device_id) == str(device_id):
                raise forms.ValidationError(
                    'Hostname not equal to \'hostname_prefix\' + \'device_id\'. '
                    'Got %s<>%s%s' % (real_hostname, hostname_prefix, device_id,))

        if device_id == 0:
            if not real_hostname == hostname_prefix:
                raise forms.ValidationError(
                    'Hostname not equal to \'hostname_prefix\' where device_id is \'0\'. '
                    'Got %s<>%s' % (real_hostname, hostname_prefix,))
