...
#     def test_init(self):
#         """Assert only accepts a list or tuple for fields"""
#         fields = 1
...
#         self.assertTrue(isinstance(SupplementalFields(fields, 0.1), SupplementalFields))
    ...
    def test_init2(self):
        ...
        self.assertEqual(fields, supplemental_fields.supplemental_fields)
        self.assertEqual(probability, supplemental_fields.probability_to_include)
        self.assertEqual(group, supplemental_fields.group)
        self.assertEqual(grouping_field, supplemental_fields.grouping_field)
    ...
    def test_update_history_before_save1(self):
        ...
        for adm in admin.site._registry.itervalues():
            if isinstance(adm, TestScheduledModelAdmin):
                ...
                adm.supplemental_fields.probability_to_include = 0.0  # so it will exclude the supplemental fields
                adm.add_view(request)
                self.assertEqual(adm.grouping_field, 'test_visit')
                self.assertEqual(adm.grouping_pk, fake_visit_pk)
                self.assertEqual(adm.exclude_fields, ('f2', 'f3', 'f4'))
    ...
    def test_update_history_before_save2(self):
        ...
        for adm in admin.site._registry.itervalues():
            if isinstance(adm, TestScheduledModelAdmin):
                ...
                request = self.factory.get('/', data={'visit_attr': 'test_visit', 'test_visit': fake_visit_pk})
                request.user = self.user
                adm.supplemental_fields.probability_to_include = 1.0  # so the fields will be included
                adm.add_view(request)
                self.assertEqual(adm.grouping_field, 'test_visit')
                self.assertEqual(adm.grouping_pk, fake_visit_pk)
                self.assertEqual(adm.exclude_fields, '')
                self.assertEqual(ExcludedHistory.objects.all().count(), 1)
    ...
    def test_update_history_after_save1(self):
        ...
        for adm in admin.site._registry.itervalues():
            if isinstance(adm, TestScheduledModelAdmin):
                ...
                adm.supplemental_fields.probability_to_include = 0.0

                # call change view to fake a change through admin
                adm.change_view(request, test_scheduled_model.pk)

                self.assertEqual(adm.grouping_field, 'test_visit')
                self.assertEqual(adm.grouping_pk, self.test_visit.pk)
                self.assertEqual(adm.exclude_fields, ('f2', 'f3', 'f4'))
                ...
                self.assertEqual(ExcludedHistory.objects.filter(grouping_pk=adm.grouping_pk).count(), 1)
    ...
    def test_update_history_after_save2(self):
        ...
        for adm in admin.site._registry.itervalues():
            if isinstance(adm, TestScheduledModelAdmin):
                ...
                adm.supplemental_fields.probability_to_include = 1.0
    ...
    def test_check_supplemental_in_original(self):
        ...
        supplemental_fields.original_model_admin_fields = ('field1', 'field2', 'field3', 'field4', 'field5')
        ...
        supplimental_fields.original_model_admin_fields = ('field1', 'field2', 'field3', 'field4', 'field5')
    ...
    def test_check_supplemental_field_attrs(self):
        ...
        supplemental_fields.model = BadModel1
        self.assertRaises(ImproperlyConfigured, supplemental_fields.check_supplemental_field_attrs)
        ...
        supplemental_fields.model = BadModel2
        self.assertRaises(ImproperlyConfigured, supplemental_fields.check_supplemental_field_attrs)
        ...
        supplemental_fields.model = BadModel3
        self.assertRaises(ImproperlyConfigured, supplemental_fields.check_supplemental_field_attrs)
...
#             self.assertEqual(sf._probability_as_sequence.count(0), opt[INC])
#             self.assertEqual(sf._probability_as_sequence.count(1), opt[EXC])
#             zero = []
#             one = []
